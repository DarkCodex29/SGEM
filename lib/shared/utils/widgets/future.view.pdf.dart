...
Widget rxFutureViewPdf(BuildContext context, Rxn<List<PdfPageImage?>> futurePdf, double angleRotation) {
    final screenHeigth = MediaQuery.of(context).size.height;
    if (futurePdf.value == null){
      return const Center(child: CircularProgressIndicator());
    } else  {
          // Agrupamos las imágenes en pares
          List<List<PdfPageImage?>> rows = [];
          for (int i = 0; i < futurePdf.value!.length; i += 2) {
            final range = i + 2 > futurePdf.value!.length ? futurePdf.value!.length : i + 2;
            rows.add(futurePdf.value!.sublist(i, range));
          }
          return Column(
            children: [
              SizedBox(
              ...
              Row(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: (){
                        descargarPaginasComoPdf(futurePdf.value!);
                    },
                    style: ElevatedButton.styleFrom(
                      padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 16),
                      backgroundColor: Colors.blue,
